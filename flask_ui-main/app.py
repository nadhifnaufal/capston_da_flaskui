...
def index():
    ...
    # Statistik
    top_category = top_category = pd.crosstab(index = df2.Category, columns = 'Jumlah', values= df2.Category, aggfunc='count').sort_values(by='Jumlah', ascending=False).reset_index().head()
    ...
    dftask10 = playstore.copy()
    dftask10.Type = dftask10.Type.astype('category')

    newdftask10 = dftask10.groupby(['Category','Type']).agg({
    'Installs' : 'count'
    }).reset_index()
    ...
    # # bagian ini digunakan untuk membuat bar plot
    cat_free_paid = newdftask10.pivot(index='Category', columns='Type', values='Installs').sort_values('Paid', ascending=False).head(5).plot.bar()

    
    # bagian ini digunakan untuk menyimpan plot dalam format image.png
    plt.savefig('cat_free_paid.png',bbox_inches="tight") 
